    ...
    $scope.doLogin = function(event) {
        ...
        $timeout(function(){
            if ($scope.userLoginForm.$invalid) {
                showFormMessage.error($scope.userLoginForm, '#notification');
            } else {
    ...
    $scope.createNewAccount = function() {
        User.add(angular.toJson($scope.facebookData)).then(function(user){
            if (user.User != undefined) {
                ...
                $("#formMessage").removeClass('success').removeClass('error').addClass('error').show().html('').html('<ul><li>' + 'Can not register with your Facebook information.' + '</li></ul>').focus();
                $( 'html, body' ).animate( { scrollTop: 0 }, 'slow' );
            }
        });
    };
            ...
            if (grants) {
                FB.api('/me', function(fbdata) {
                    if (!fbdata || fbdata.error || !fbdata.email) {
                        ...
                        $scope.facebookData = { email : fbdata.email, fullname :  fbdata.name,facebook_id : fbdata.id };  
                ...
                console.log(r);
                $("#formMessage").removeClass('success').removeClass('error').addClass('error').show().html('').html('<ul><li>You need to allow email permission in order to using this feature</li></ul>').focus(); 
                $( 'html, body' ).animate( { scrollTop: 0 }, 'slow' );
            }
        });
